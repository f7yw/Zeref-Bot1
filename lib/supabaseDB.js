...
const SYNC_INTERVAL = 30 * 1000   // مزامنة كل 30 ثانية للحفاظ على البيانات أسرع في السحاب
...
export class SupabaseDB {
  ...
  // تكتب كل دورة دون انتظار علم _dirty (لأن أغلب الإضافات لا تستدعي markDirty)
  _startAutoSync() {
    ...
    this._syncTimer = setInterval(async () => {
      try {
        ...
        await this.write()
        // طباعة مختصرة كل 5 دورات لتجنّب السبام
        this._tickCount = (this._tickCount || 0) + 1
        if (this._tickCount % 10 === 0) {
          const u = Object.keys(this.data?.users || {}).length
          const c = Object.keys(this.data?.chats || {}).length
          console.log(`[DB] ☁️  Auto-sync ✅  (users=${u}, chats=${c}, errors=${this._syncErrors})`)
        }
      } catch (e) {
        console.error('[DB] Auto-sync exception:', e?.message)
      }
    }, SYNC_INTERVAL)
