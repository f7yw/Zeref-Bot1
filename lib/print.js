...
function parseJid(jid) {
  ...
  const pn = PhoneNumber('+' + raw)
  return pn.isValid() ? pn.getNumber('international') : ('+' + raw)
}
let _name = await conn.getName(m.sender)
let sender = parseJid(m.sender) + (_name ? ' ~' + _name : '')
...
sender,
m ? (m.exp || 0) : '?',
user ? '|' + (user.exp || 0) + '|' + (user.limit || 0) : '|0|0',
m.chat + (chat ? ' ~' + chat : ''),
