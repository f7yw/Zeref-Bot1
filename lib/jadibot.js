...
import { notifySubBotEvent } from './notify.js'
...
export async function createSubBot(phone, ownerJid = null) {
  ...
  try {
    ...
    if (existsSync(credsFile)) {
      ...
      // ⚠️ المهم: creds.me يُكتَب فور طلب كود الإقران (قبل أن يُدخله المستخدم).
      // دليل الإقران الفعلي هو creds.registered === true + وجود account (يُحقَن
      // فقط بعد نجاح المصافحة الكاملة في messages-recv.js داخل Baileys).
      const isFullyPaired = parsed?.registered === true && !!parsed?.account
      if (!isFullyPaired) {
        for (const f of readdirSync(folder)) {
        ...
        global.db?.markDirty?.()
        console.log(`[JADIBOT] 🧹 Cleared incomplete creds for +${num} (registered=${parsed?.registered}, hasAccount=${!!parsed?.account}). Starting fresh.`)
      }
    }
  } catch (e) {
  ...
  // ⚠️ مهم: استخدم makeWASocket الموسَّع من lib/simple.js وليس من Baileys مباشرة.
  // الموسَّع يضيف pushMessage / smsg / decodeJid / reply / sendFile … وهي ضرورية
  // كي يعمل handler.js عبر البوت الفرعي بنفس الكفاءة.
  const { makeWASocket } = await import('./simple.js')
  ...
  subConn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      const wasReconnect = subConn.__hasConnectedOnce
      subConn.__hasConnectedOnce = true
      subConn.__reconnectAttempts = 0
      console.log(`[JADIBOT] ✅ Sub-bot ${num} connected.`)
      persistSessionToCloud(num)
      if (!wasReconnect) {
        notifySubBotEvent(num, 'connected', { features: getSubBotFeatures(num) }).catch(() => {})
      }
    }
    if (connection === 'close') {
      ...
      if (subConn.__supersededBy) {
        ...
        notifySubBotEvent(num, 'superseded').catch(() => {})
        return
      }

      // هل الجلسة مُسجَّلة فعلياً (مرّت بالـ pairing كاملاً)؟
      const credsNow = subConn.authState?.creds
      const wasFullyPaired = !!(credsNow?.registered && credsNow?.account)

      // 401:
      //   - لجلسة لم تُسجَّل بعد   → نافذة الإقران انتهت ولم يدخل المستخدم الكود → نُبقي الجلسة لإعادة المحاولة
      //   - لجلسة مُسجَّلة فعلياً    → واتساب أبطل الجلسة (تسجيل خروج من الهاتف / استبدال بجلسة أخرى) → تنظيف كامل
      if (code === 401) {
        if (wasFullyPaired) {
          console.log(`[JADIBOT] ❌ Sub-bot ${num} session invalidated by WhatsApp (logged out). Cleaning up.`)
          delete sessionsEntry()[num]
          global.db?.markDirty?.()
          notifySubBotEvent(num, 'loggedOut', { code }).catch(() => {})
          await destroySubBot(num).catch(() => {})
          return
        }
        console.log(`[JADIBOT] ⏰ Sub-bot ${num} pairing window expired (401 before pairing completed). Session preserved.`)
        global.conns = global.conns.filter(c => c !== subConn)
        return
      }

      // إزالة الاتصال القديم من القائمة الحية
      global.conns = global.conns.filter(c => c !== subConn)

      // أكواد تستوجب إعادة اتصال فوري (الجلسة سليمة لكن البروتوكول طلب إعادة بدء):
      //   515 = Stream Errored — Restart Required (يحدث بعد أول إقران ناجح)
      //   428 = Connection Closed
      //   408 = Connection Lost / Timeout
      //   500 = Bad Session / صيانة
      //   503 = Service Unavailable
      const RECONNECT_CODES = new Set([408, 428, 500, 503, 515])
      // 440 = Connection Replaced (جلسة أخرى استبدلتنا) → لا تُعِد الاتصال
      // 403 = Forbidden → لا تُعِد، يحتاج تدخّل
      const NO_RECONNECT_CODES = new Set([403, 440])

      if (NO_RECONNECT_CODES.has(code)) {
        console.log(`[JADIBOT] 🚫 Sub-bot ${num} closed (code=${code}). No auto-reconnect.`)
        return
      }

      if (wasFullyPaired && (RECONNECT_CODES.has(code) || code === undefined)) {
        const attempts = (subConn.__reconnectAttempts || 0) + 1
        if (attempts > 5) {
          console.log(`[JADIBOT] ⛔ Sub-bot ${num} reached max reconnect attempts (5). Will retry on next app restart.`)
          notifySubBotEvent(num, 'failed', { code, error: 'تجاوز الحدّ الأقصى لمحاولات إعادة الاتصال (5).' }).catch(() => {})
          return
        }
        const delay = Math.min(1500 * attempts, 8000)
        console.log(`[JADIBOT] 🔄 Sub-bot ${num} reconnecting in ${delay}ms (attempt ${attempts}/5, code=${code})...`)
        notifySubBotEvent(num, 'reconnecting', { code, attempts, delayMs: delay }).catch(() => {})
        setTimeout(async () => {
          try {
            const r = await createSubBot(num, subConn.__subBotOwner)
            if (r?.ok) {
              const fresh = global.conns.find(c => c.__subBotPhone === num)
              if (fresh) fresh.__reconnectAttempts = attempts
            }
          } catch (e) {
            console.error(`[JADIBOT] auto-reconnect ${num} failed:`, e?.message)
          }
        }, delay)
        return
      }

      console.log(`[JADIBOT] ⚠️ Sub-bot ${num} closed (code=${code}, paired=${wasFullyPaired}). Session kept; will reconnect on restart.`)
      notifySubBotEvent(num, 'disconnected', { code }).catch(() => {})
    }
  })
...
export async function restoreAllSubBots() {
  ...
  for (const dir of readdirSync(JADIBOT_DIR)) {
    ...
    try {
      ...
      const isFullyPaired = parsed?.registered === true && !!parsed?.account
      if (!isFullyPaired) {
        console.log(`[JADIBOT] ⏭️ Skipping ${dir} — pairing not completed (registered=${parsed?.registered}).`)
        continue
      }
