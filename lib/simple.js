        ...
        processMessageStubType: {
            ...
            async value(m) {
                ...
                const emitGroupUpdate = (update) => {
                    try { conn.ev.emit('groups.update', [{ id: chat, ...update }]) } catch (_) {}
                }
                switch (m.messageStubType) {
                    ...
                    case WAMessageStubType.GROUP_CHANGE_SUBJECT:
                        emitGroupUpdate({ subject: m.messageStubParameters[0] })
                        break
                    case WAMessageStubType.GROUP_CHANGE_DESCRIPTION:
                        emitGroupUpdate({ desc: m.messageStubParameters[0] })
                        break
                    case WAMessageStubType.GROUP_CHANGE_ANNOUNCE:
                        emitGroupUpdate({ announce: m.messageStubParameters[0] === 'on' || m.messageStubParameters[0] === 'true' })
                        break
                    case WAMessageStubType.GROUP_CHANGE_RESTRICT:
                        emitGroupUpdate({ restrict: m.messageStubParameters[0] === 'on' || m.messageStubParameters[0] === 'true' })
                        break
                    default: {
                        console.log({
