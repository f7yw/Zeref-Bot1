...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        /**
         * إرسال رسالة تفاعلية (أزرار + قائمة nativeFlow)
         * @param {string} jid - JID المحادثة
         * @param {{ body, footer, header, buttons }} opts
         *   body    - نص الرسالة الرئيسي
         *   footer  - نص التذييل (اختياري)
         *   header  - { title } (اختياري)
         *   buttons - مصفوفة أزرار Baileys nativeFlowMessage
         *             [{ name: 'quick_reply', buttonParamsJson: '{"display_text":"...","id":"..."}' }]
         * @param {object} quoted  - رسالة مقتبسة (اختياري)
         * @param {object} options - خيارات إضافية (اختياري)
         */
        sendInteractive: {
            /**
             * الصيغة: viewOnceMessage wrapper + proto.Message.fromObject خارجي
             * هذه هي الصيغة الوحيدة التي تُظهر الأزرار بشكل صحيح مع Baileys v7
             */
            async value(jid, { body, footer = '', header = null, buttons = [] } = {}, quoted, options = {}) {
                const interactiveMsg = proto.Message.InteractiveMessage.fromObject({
                    body:   { text: body || '' },
                    footer: { text: footer },
                    header: header
                        ? { title: header.title || '', hasMediaAttachment: false }
                ...
                const generated = generateWAMessageFromContent(jid, msgContent, {
        ...
        /**
         * إرسال قائمة منسدلة عبر proto.Message.ListMessage مباشرة (أكثر توافقاً من sendMessage)
         * @param {string} jid
         * @param {{ text, footer, title, buttonText, sections }} opts
         *   sections - [{ title, rows: [{ title, rowId, description }] }]
         * @param {object} quoted
         */
        sendList: {
            async value(jid, { text, footer = '', title = '', buttonText = 'اختر', sections = [] } = {}, quoted, options = {}) {
                const msgContent = proto.Message.fromObject({
        ...
        /**
         * إرسال رسالة بأزرار (buttonsMessage — صيغة أقدم لكن توافق أوسع)
         * @param {string} jid
         * @param {{ text, footer, buttons }} opts
         *   buttons - [{ id, text }] — id هو معرّف الأمر، text هو نص الزر
         * @param {object} quoted
         */
        sendButtons: {
